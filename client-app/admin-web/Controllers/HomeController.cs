using System.Diagnostics; using Microsoft.AspNetCore.Mvc; using AdminWeb.Models; using AdminWeb.Services; using AspNetCoreHero.ToastNotification.Abstractions; using Microsoft.Extensions.Primitives; using Microsoft.AspNetCore.Authorization;  namespace AdminWeb.Controllers;  public class HomeController : Controller {     private readonly ILogger<HomeController> _logger;     private readonly INotyfService _notyfService;     private readonly ToastMessageService _toastMsgService;
    ...
    public HomeController(         ILogger<HomeController> logger,         INotyfService notyfService,         ToastMessageService toastMsgService,         AuthService authService)     {         _logger = logger;         _notyfService = notyfService;         _toastMsgService = toastMsgService;         _authService = authService;     }      public IActionResult Index()     {
    ...
    public async Task<IActionResult> ChangePassword(IFormCollection formCol)
    {
        ...
        return Json(null);
    }      [ResponseCache(Duration = 0, Location = ResponseCacheLocation.None, NoStore = true)]     [HttpPost]     [HttpGet]     public IActionResult Error([FromForm] string message)     {         string? redirectUrl = null;         var errCode = string.Empty;         if (Request.QueryString.HasValue)
        {
